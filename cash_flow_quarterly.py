...
data = pd.read_sql("SELECT TOP(100000) * FROM Stonks.dbo.ticker_symbols", conn)
#links = data['Links']
...
for ticker,name in zip(ticker,ticker_names):
    ...
    url2 = "https://www.macrotrends.net/stocks/charts/" + ticker + "/" + name + "/cash-flow-statement?freq=Q"
    pff = pff + 1
    ##if pff == 20:
        ##break
    count += 1        
...
df.to_csv(r'C:\Users\roboz.DESKTOP-F86F289\Desktop\Statements\cash_flow_final_quarterly.csv')