...
for ticker in ticker_symbols: 
    ...
    for key,value in get_analysts_info(ticker).items():
        list_values.append(value)
    i = 0
    for x in list_values:
        list_values[i]['ticker'] = [ticker] *len(list_values[i])
        i +=1
    i = 0
    try:
        
        for x in list_values:
            df = pd.concat([df,list_values[0]])
            df_1 = pd.concat([df_1,list_values[1]])
            df_2 = pd.concat([df_2,list_values[2]])
            df_3 = pd.concat([df_3,list_values[3]])
            df_4 = pd.concat([df_4,list_values[4]])
            df_5 = pd.concat([df_5,list_values[5]])
            #df_6 = pd.concat([df_6,list_values[6]])
    except:
        pass
